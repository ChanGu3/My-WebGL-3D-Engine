...
import mat4, {UniqueMatrix} from "../../LinearAlgebra/Mat4";
...
class ShaderProgram {
    ...
    public Load():void {
        ...
        this.setUniform_Mat4x4(new mat4(UniqueMatrix.Identity)); // model view set to the Identity
    }
    ...
    public setVertexAttributesToBuffer(vertexData:VertexShaderFieldAttributes):void {
        let interleavedLength:number = 0;
        interleavedLength = (VertexShaderFieldAttributes.COORDINATES & vertexData) ? interleavedLength + ShaderProgram.coord_count : interleavedLength;
        interleavedLength = (VertexShaderFieldAttributes.COLOR & vertexData) ? interleavedLength + 4 : interleavedLength;

        let currOffset:number = 0;

        if(VertexShaderFieldAttributes.COORDINATES & vertexData) {
        ...
        if(VertexShaderFieldAttributes.COLOR & vertexData) {
